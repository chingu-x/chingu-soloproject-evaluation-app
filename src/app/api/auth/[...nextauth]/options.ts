...
export const options: NextAuthOptions = {
  ...
  callbacks: {
    ...
    async signIn({ user }) {
      ...
      user.evaluatorEmail =
        userFromDb.evaluatorEmail as string;

      // check if user has the allowed roles and active
      const allowedRoles: ChinguAppRole[] = [
        'admin',
        'evaluator',
      ];
      ...
      ); // only allow access for Active staff with a record on airtable
    },
    ...
    async jwt({ token, user }) {
      if (user) {
        console.log(
          `[jwt] user: ${user.email} just logged in.`
        );
        ...
        token.evaluatorEmail =
          user.evaluatorEmail as string;
        token.permissionLastChecked = Date.now();
      } else {
        console.log(
          `[jwt] user ${token.email} already logged in. Last permission check: ${new Date(token.permissionLastChecked!).toString()} `
        );
        const checkInterval = 60 * 60 * 1000; // 1h
        const shouldRefreshPermissions =
          !token.permissionLastChecked ||
          Date.now() >
            token.permissionLastChecked + checkInterval;

        if (shouldRefreshPermissions) {
          console.log(
            `[jwt] refresh permissions - ${new Date().toString()}`
          );
          ...
          try {
            ...
            if (userInDb) {
              token.roles =
                userInDb.roles as ChinguAppRole[];
              token.evaluatorEmail =
                userInDb.evaluatorEmail as string;
            }
            token.permissionLastChecked = Date.now();
          } catch (error) {
            console.error(
              'Failed to refresh permission: ',
              error
            );
          }
        }
